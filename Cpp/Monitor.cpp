...
bool DirMonitor::AddDir(const std::wstring& dir, std::function<void(const std::wstring&)> func)
{
	...
	/*bool bExist = false;
	std::for_each(std::begin(dirs), std::end(dirs),
		[&bExist, dir](std::pair<std::wstring, HANDLE>& pair) {
		bExist = boost::iequals(pair.first, dir);
	});
...
void DirMonitor::_threadwork()
{
	...
	{
		DWORD waitstatus = ::WaitForMultipleObjects(hs.size(), hs.data(), false, waitSecs);
